...
export const ApparelMessage = ({ data }) => {
  const [apparelData, setApparelData] = useState(data);
  const { _id, orderStatus, email, readStatus, customApparel } = apparelData;
  const [uploadFiles, setUploadFiles] = useState(customApparel.completeOrder);
  ...
  const handleDownloadAllAsZip = async () => {
    ...
    const fetchPromises = customApparel.attachment.map(async (item, index) => {
      try {
        ...
        zip.file(`attachment_${item.name}.${extension}`, blob);
      } catch (error) {
